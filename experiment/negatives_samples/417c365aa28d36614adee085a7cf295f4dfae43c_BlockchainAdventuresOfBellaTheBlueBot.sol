...
contract BlockchainAdventuresOfBellaTheBlueBot is ERC721Enumerable, Ownable {
    ...
    ) ERC721("BlockchainAdventuresOfBellaTheBlueBot", "BELLA") {
    ...
    function remainingPresaleAmount(
        address _address
    ...
    function mintReserved(address _to, uint256 _mintAmount) external onlyOwner {
        ...
        require(
            reserved - _mintAmount >= 0,
            "You cannot mint more tokens than the amount reserved"
        );
    ...
    function mintPresale(uint256 _mintAmount) external payable {
        ...
        require(
            _mintAmount > 0 &&
                _mintAmount <= presaleAddressesToRemainingAmount[msg.sender],
            "Invalid mint amount"
        );

        require(
            cost * _mintAmount <= msg.value,
            "Value of transaction is too low"
        );
        require(
            supply + reserved + _mintAmount <= maxSupply,
            "Transaction would exceed max supply"
        );
    ...
    function mintPublicSale(uint256 _mintAmount) external payable {
        ...
        require(
            balanceOf(msg.sender) + _mintAmount <= 100,
            "You cannot mint more than 100 tokens"
        );
        require(
            supply + reserved + _mintAmount <= maxSupply,
            "Transaction would exceed max supply"
