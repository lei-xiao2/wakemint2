...
contract ElderGODS is ERC721Enumerable, Ownable, IERC4907, ReentrancyGuard {
    ...
    function airdropSloth(uint256 gasLimit) external onlyOwner nonReentrant {
        ...
        for (
            uint256 x = slothAirdropTokenCount + 1;
            x <= IERC721Enumerable(INFINIGODS).totalSupply() &&
                gasleft() > gasLimit;
    ...
    function airdropStandard(uint256 gasLimit) external onlyOwner nonReentrant {
        address to;
        for (
            uint256 x = normalAirdropTokenCount + 1;
            x <= IERC721Enumerable(INFINIGODS).totalSupply() &&
                gasleft() > gasLimit;
    ...
    function mint(
        ...
        require(
            totalSupply() + amount <= MAX_MINT &&
                totalSupply() + amount <= MAX_SUPPLY,
            "GODS: Supply would be exceeded"
        );
        require(
            block.timestamp >= sale_start_time,
            "GODS: Mint has not started"
        );
        ...
        for (uint256 i; i < _tokenId.length; ) {
            require(
                IERC721(INFINIGODS).ownerOf(_tokenId[i]) == _msgSender(),
                "GODS: Caller not owner of token"
            );
    ...
    function getAvailable(
        ...
        for (uint i = 0; i < _tokenIds.length; ) {
            if (
                IInfiniGods(INFINIGODS).getTokenRarity(_tokenIds[i]) == 0 &&
                !hasMinted[_tokenIds[i]]
            ) {
                result[i] = 1;
            }
            unchecked {
    ...
    function getUserTokens(
        ...
        for (uint i = 0; i < balance; ) {
            result[i] = IERC721Enumerable(INFINIGODS).tokenOfOwnerByIndex(
                user,
                i
            );
            unchecked {
    ...
    uint256 private rentBidsCounter;
    ...
    function _setUser(uint256 tokenId, address user, uint64 expires) internal {
        _requireMinted(tokenId);
        require(
            rent[tokenId].user == address(0) ||
                rent[tokenId].expires < block.timestamp,
            "GODS: Currently Rented"
        );
    ...
    function acceptRent(uint256 _tokenId) external rentability nonReentrant {
        ...
        require(
            ...
            "GODS: Cannot rent own token"
        );
        require(
            ownerOf(_tokenId) == rentableItems[_tokenId].lender,
            "GODS: Lender cannot rent"
        );
        require(
            rentableItems[_tokenId].expiry >= block.timestamp,
            "GODS: Offer expired"
        );
        ...
        if (godsFee > 0) {
            ...
            IERC20(rentableItems[_tokenId].payToken).safeTransferFrom(
                _msgSender(),
                ownerOf(_tokenId),
                toOwner
            );
        ...
        _setUser(
            _tokenId,
            _msgSender(),
            uint64(block.timestamp + rentableItems[_tokenId].duration)
        );
    }
    ...
    function acceptBid(uint256 bidId) external rentability nonReentrant {
        RentBids memory bid = rentBids[bidId];
        require(
            ownerOf(bid.tokenId) == _msgSender(),
            "GODS: Caller is not the owner"
        );
        require(
            rent[bid.tokenId].user == address(0) ||
                rent[bid.tokenId].expires < block.timestamp,
            "GODS: Currently Rented"
        );
        ...
        if (godsFee > 0) {
            uint256 toOwner = (cost * (denominator - godsFee)) / denominator;
            uint256 toTreasury = (cost * (godsFee)) / denominator;
            IERC20(bid.payToken).safeTransferFrom(
                bid.user,
                ownerOf(bid.tokenId),
                toOwner
            );
