...
contract CHISRewards is ICHISRewards, Ownable {
    ...
    function depositRewards() external payable override {
        ...
        require(
            ...
            "must be shares to distribute rewards"
        );
        ...
        if (amount > 0) {
            ...
            if (compound) {
                ...
                uniswapV2Router.swapExactETHForTokens{value: amount}(
                    (_tokensToReceiveNoSlip * (100 - compoundBuySlippage)) /
                        100, // handle slippage
                    path,
                    shareholder,
                    block.timestamp
    ...
    function _getTokensToReceiveOnBuyNoSlippage(
        uint256 _amountETH
    ...
    function unstake(uint256 _tokenId) public {
        ...
        if (_isUnstakingEarly) {
            ...
            if (address(stakeToken) == address(CHI)) {
                ...
                stakeToken.transfer(
                    owner(),
                    _tokenStake.amountStaked - _earnedAmount
                );
            }
        } else {
...
contract xlCHI is CHIStake {
    ...
    function _getETHToReceiveOnSellNoSlippage(
        uint256 _amountCHI
    ) internal view returns (uint256) {
        IUniswapV2Pair pair = IUniswapV2Pair(address(stakeToken));
        (uint112 _r0, uint112 _r1, ) = pair.getReserves();
    ...
    function _returnExcesxCHI(address _user, uint256 _initialBal) internal {
        ...
        if (_currentBal > _initialBal) {
            ...
            require(
                CHI.balanceOf(address(this)) >= _initialBal,
                "took too much"
            );
        }
    }
...
contract CHI is ICHI, ERC20, Ownable {
    ...
    function _transfer(
        ...
        if (_isSell && !(isTaxExcluded[sender] || isTaxExcluded[recipient])) {
            ...
            uint256 _taxEarlyPerc = getSellEarlyTax(
                sender,
                amount,
                taxEarlyPerc
            );
            uint256 _totalTax = taxMarketingPerc +
                taxRewardsPerc +
                _taxEarlyPerc;
            tax = (amount * _totalTax) / 10000;
            if (tax > 0) {
                uint256 _taxAmountETH = (tax *
                    (taxMarketingPerc + taxRewardsPerc)) / _totalTax;
    ...
    function _swapForETH(uint256 _amountToSwap) private swapLock {
        ...
        if (_balToProcess > 0) {
            ...
            // send marketing ETH
            uint256 _marketingETH = (_balToProcess * taxMarketingPerc) /
                _totalTaxETH;
    ...
    function getSellEarlyExpiration(
        address _seller
    ) public view returns (uint256) {
        return
...
contract CHIVester is Ownable {
    ...
    function withdraw(uint256 _index) external {
        ...
        uint256 _withdrawsToComplete = _withdrawsAllowed -
            _vest.withdrawsCompleted;
    ...
    function getUserVests(
        address _user
    ) external view returns (TokenVest[] memory) {
        return vests[_user];
    }
