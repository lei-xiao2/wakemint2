...
contract NFTEasyDrop {
    ...
    modifier isEligible() {
        require(
            subscribers[msg.sender].subscribed ||
                msg.value >= txFee ||
                msg.sender == owner,
            "Not subscribed or trying to pay less than the fee size"
        );
        _;
    }
    ...
    function subscribe() external payable {
        ...
        require(
            msg.value >= subscriptionFees[0],
            "Trying to pay less than minimum subscription fee"
        );
        received += msg.value;
    ...
    function removeSub(address _sub) external onlyOwner {
        require(
            subscribers[_sub].subscribed &&
                subscribers[_sub].until < block.timestamp,
            "Not subscribed or subscription is not expired yet"
        );
        subscribers[_sub].subscribed = false;
    }

    function removeAllExpiredSubs(
        address[] calldata _subscribers
    ) external onlyOwner {
        for (uint i = 0; i < _subscribers.length; i++) {
            if (
                subscribers[_subscribers[i]].subscribed &&
                subscribers[_subscribers[i]].until < block.timestamp
            ) subscribers[_subscribers[i]].subscribed = false;
        }
    }
    ...
    function airdrop1155(
        ...
        require(isApproved(_token), "Token not approved");
        require(
            _to.length == _id.length && _to.length == _amount.length,
