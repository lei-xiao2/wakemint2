...
contract PixelShibs is ERC721Enumerable, Ownable {
    ...
    address payable private _PaymentAddress =
        payable(0x8B71093a446881a96cEe3a682d83C1c6A58a7034);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicPIXELSHIB.current() < PIXELSHIB_MAX,
            "Purchase would exceed PIXELSHIB_MAX"
        );
        ...
        if (msg.sender != owner()) {
            if (_mappingWhiteList[msg.sender] == false) {
                ...
                require(
                    block.timestamp > _activeDateTime - PRESALE_HOURS,
                    "Contract is not active for presale"
                );
                require(
    ...
    function tokenURI(
        ...
        if (_mappingMintTimestamp[tokenId] + REVEAL_DELAY < block.timestamp) {
            if (tokenId >= 0 && tokenId < 5000) {
                return
                    string(
                        abi.encodePacked(_tokenBaseURI1, tokenId.toString())
                    );
            } else if (tokenId >= 5000 && tokenId < 10000) {
                return
                    string(
                        abi.encodePacked(_tokenBaseURI2, tokenId.toString())
                    );
            } else if (tokenId >= 10000 && tokenId < 15000) {
                return
                    string(
                        abi.encodePacked(_tokenBaseURI3, tokenId.toString())
                    );
            } else if (tokenId >= 15000 && tokenId < 20000) {
                return
                    string(
                        abi.encodePacked(_tokenBaseURI4, tokenId.toString())
                    );
            } else if (tokenId >= 20000 && tokenId < 25000) {
                return
                    string(
                        abi.encodePacked(_tokenBaseURI5, tokenId.toString())
                    );
            } else if (tokenId >= 25000 && tokenId < 30000) {
                return
                    string(
                        abi.encodePacked(_tokenBaseURI6, tokenId.toString())
                    );
            }
        }
