...
contract THLC is ERC721Enumerable, Ownable {
    ...
    function mint(uint chosenAmount) public payable isSaleOpen {
        require(
            _TOTALSUPPLY >= chosenAmount,
            "Chosen amount is greater than supply"
        );
        ...
        require(
            chosenAmount <= publicSaleMaxQuantity && chosenAmount > 0,
            "Chosen Amount Should be greater then 0 and less then max quantity"
        );
        require(
            publicSaleprice.mul(chosenAmount) == msg.value,
    ...
    function whiteListMint(
        ...
        require(
            preSalePrice.mul(chosenAmount) == msg.value,
            "Sent Ether Value Is Incorrect"
        );
