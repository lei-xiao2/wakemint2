...
contract MutantNotOkayBears is ERC721A, Ownable {
    ...
    constructor(
        uint256 maxTokens_,
        bool saleEnabled_,
        bool freeMintEnabled_,
        string memory baseURI_,
        uint256 maxMintForUser_,
        uint256 maxFreeMints_,
        uint256 price_
    ) ERC721A("Mutant Not Okay Bears", "Mutant Bear") {
    ...
    function hasFreeMint(address target) public view returns (bool) {
        return
            _freeMintEnabled &&
            freeMints[target] < _maxMintForUser &&
            _currentFreeMints < _maxFreeMints;
    }
    ...
    function freeMint(uint256 count) external {
        ...
        require(
            freeMints[msg.sender] + count <= _maxMintForUser,
            "You have max tokens"
        );
        require(
            _currentFreeMints + count <= _maxFreeMints,
            "You have max tokens"
        );
        _safeMint(msg.sender, count);
    ...
    function mintServer(
        ...
        bytes32 message = prefixed(
            keccak256(abi.encodePacked(msg.value, msg.sender, count, salt))
        );
    ...
    function mintServer(
        ...
        bytes32 message = prefixed(
            keccak256(abi.encodePacked(msg.value, msg.sender, count, uri, salt))
        );
        require(recoverSigner(message, signature) == serverAddress, "Error");
        signatureUsed[signature] = true;
    ...
    function tokenURI(
        ...
        if (bytes(customTokensURIs[tokenId]).length != 0)
            return customTokensURIs[tokenId];
        string memory baseURI = _baseURI();
        return
            bytes(baseURI).length > 0
