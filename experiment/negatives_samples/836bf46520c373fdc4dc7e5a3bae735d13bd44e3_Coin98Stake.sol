...
contract Coin98Stake is ERC721Enumerable, Ownable {
    ...
    ) ERC721("Coin98 Staking Certificate", "C98SC") {
        setBaseURI(baseURI);
    }
    ...
    function validPackageCondition(
        uint256 _min,
        uint256 _max,
        uint256 _time,
        uint256 _rate,
        ...
        require(
            _min > 0 && _max > 0 && _min < _max,
            "C98Stake: Wrong numeric format"
        );
        MetaInfo memory metaInfo = MetaInfos[_meta];

        require(
            _time > metaInfo.locked_time && _rate > metaInfo.floating_rate,
            "C98Stake: Lower than minimum time & rate"
        );
    }
     ...
     * - `package` must be existed.
     */
    function unRegister(string memory _package) public onlyOwner {
    ...
    function getStakedByTokenId(
        ...
        if (!stakeInfo.flag || (timeStaked < metaInfo.locked_time)) {
            ...
            uint256 calRate = timeStaked < stakeInfo.package_time
                ? metaInfo.floating_rate
                : stakeInfo.rate;

            uint256 amountProfitBySeconds = stakeInfo
                .amount
                .div(Percent)
                .div(yearTimestamp)
                .mul(calRate);
    ...
    function validateCustomName(
        ...
        if (prefixNameSize == 0) {
            return
                Convertible.compareStrings(
                    Convertible.sliceString(
                        1,
                        10,
                        Convertible.convertAddressToString(user)
                    ),
                    name
                );
        }
        return
            Convertible.compareStrings(
                Convertible.sliceString(1, prefixNameSize, name),
                name_prefix
            ) && nameSize == 10;
    }
    ...
    function stake(
        ...
        require(
            _amount >= pkInfo.min && _amount < pkInfo.max,
            "C98Stake: Wrong min max format"
        );
        ...
        require(
            totalStakedFinal <= metaInfo.max_staked,
            "C98Stake: Maximum number of staked"
        );
        ...
        bool _isNotCustomname = validateCustomName(
            _name,
            metaInfo.name_prefix,
            msg.sender
        );

        if (!_isNotCustomname) {
            require(
                nameSize <= 20 && nameSize > 0,
                "C98Stake: Not meet name condition"
            );
        }

        uint256 payAmount = _amount.add(_isCustomID ? metaInfo.id_fee : 0).add(
            _isNotCustomname ? 0 : metaInfo.naming_fee
        );
        ...
        if (_isCustomID) {
            ...
            string memory randomConvert = Convertible.uint2str(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            totalToken().add(1),
        ...
        uint256 nftPackageId = Convertible.bytesToUInt(
            Convertible.stringToBytes32(
                string(abi.encodePacked(metaInfo.nft_prefix, randomID))
            )
        );
    ...
    function renaming(uint256 _tokenId, string memory _name) public {
        ...
        require(
            ...
            "C98Stake: Not meet owner condition"
        );
        metaToken.safeTransferFrom(
            msg.sender,
            address(this),
            metaInfo.naming_fee
        );
    ...
    function claimPending(uint256 _tokenId) public {
        ...
        require(
            stakeInfo.pending_flag,
            "C98Stake: Not meet claim pending condition"
        );
        require(
            ownerOf(_tokenId) == msg.sender,
            "C98Stake: Not meet owner condition"
        ...
        require(
            metaToken.balanceOf(address(this)) >= stakeInfo.earn_staked,
            "C98Stake: Not enough pool"
        );
