...
contract NinjitSu is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    function teamMint(address[] memory to, uint quantity) external onlyOwner {
        require(
            (to.length * quantity) + totalSupply() <= MAX_SUPPLY,
            "Sold out"
        );
    ...
    function _mint(uint quantity) internal {
        require(
            quantity <= maxMintPerTx && quantity > 0,
            "Hit max mint per transaction."
        );

        if (phase == 1) {
            require(
                msg.value >= whitelistPrice * quantity,
    ...
    function switchPhase(uint256 phaseNumber) external onlyOwner {
        require(
            phaseNumber >= 0 && phaseNumber < 3,
            "Phase out of range 0 - 2"
        );
        phase = phaseNumber;
    }
