...
contract MessageToMartians is ERC721Enumerable, Ownable {
    ...
    function ownerMintSupplies(uint amount_) external onlyOwner {
        require(
            suppliesMinted + amount_ <= maxSupplies,
            "Not enough supplies."
        );
        ...
        for (uint i = 0; i < amount_; i++) {
            ...
            iTransponders(transpondersAddress).mintTransponder(
                msg.sender,
                _tokenId
            );
    ...
    function mintSupplies(
        uint amount_
    ) external payable onlySender publicMinting {
        require(
            suppliesMinted + amount_ <= maxSupplies,
            "Not enough supplies."
        );
    ...
    function writeMessage(
        ...
        require(
            IERC721(transpondersAddress).ownerOf(transponderId_) == msg.sender,
            "You don't own this transponder."
        );
        require(
            transpondersUsed[transponderId_] == 0,
            "Transponder already used to write."
        );
        require(stringLength(word_) <= maxStringLength, "Phrase is too long!");
        require(
            onlyAllowedCharacters(word_),
            "Phrase contains unallowed characters!"
        );
    ...
    function setTokenExternalUrl(
        string memory tokenExternalUrl_
    ) external onlyOwner {
    ...
    function tokenURI(
        ...
        string memory _image = string(
            abi.encodePacked(
                ...
                "<svg xmlns='http://www.w3.org/2000/svg' width='500' height='500' style='background-color:#000000'><style>@font-face{font-family:'Minecraft'; src:url('data:application/octet-stream;base64,",
                fontBase64(),
                "');</style><text fill='#00fff2' font-family='Minecraft' x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' font-size='32px'>",
                _wordString,
                '</text></svg>",'
            )
        );
        string memory _external_url = string(
            abi.encodePacked('"external_url": "', tokenExternalUrl, '",')
        );
        string memory _description = string(
            abi.encodePacked('"description": "', tokenDescription, '",')
        );
        string memory _attributes = string(
            abi.encodePacked(
                '"attributes":[{"trait_type": "type", "value": "',
                _type,
                '"}, {"trait_type": "phrase", "value": "',
                _wordString,
                '"}, '
            )
        );
        _attributes = string(
            abi.encodePacked(
                ...
                Strings.toString(_wordLength),
                '"}, {"trait_type": "message position", "value": "',
