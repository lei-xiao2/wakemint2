...
contract DiamondBase is DiamondAccessControl {
    ...
    function _isDiamondOutside(string _diamondId) internal view returns (bool) {
        ...
        return
            keccak256(
                abi.encodePacked(diamondIdToMetadata[_diamondId].status)
            ) == keccak256(abi.encodePacked(STATUS_OUTSIDE));
    }

    function _isDiamondVerified(
        string _diamondId
        ...
        return
            keccak256(
                abi.encodePacked(diamondIdToMetadata[_diamondId].status)
            ) == keccak256(abi.encodePacked(STATUS_VERIFIED));
    }
}
...
contract DiamondCore is DiamondBase721 {
    ...
    function createDiamond(
        ...
        string _gemSubcategory,
        string _media
    ) external onlyAdminOrCEO whenNotPaused {
        require(!diamondExists[_diamondId]);
