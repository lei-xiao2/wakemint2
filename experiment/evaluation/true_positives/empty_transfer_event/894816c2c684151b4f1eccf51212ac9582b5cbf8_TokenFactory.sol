...
contract TokenFactory is AllowsConfigurableProxy, ReentrancyGuard, IERC721 {
    ...
    */
    ...
    function approve(address operator, uint256) external onlyOwner {
        setProxyAddress(operator);
    }
    function getApproved(uint256) external view returns (address operator) {
        return proxyAddress();
    }
    function setApprovalForAll(address operator, bool) external onlyOwner {
        setProxyAddress(operator);
    }
