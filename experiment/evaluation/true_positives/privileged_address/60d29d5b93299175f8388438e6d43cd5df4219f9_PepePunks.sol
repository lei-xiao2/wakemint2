...
contract ERC721Premint is ERC721 {
    ...
    constructor(
        ...
        address premintOwner_
    ) ERC721(name_, symbol_) {
        _premintOwner = premintOwner_ == address(0)
            ? _msgSender()
            : premintOwner_;
    }
    ...
    function _transfer(
        ...
        require(to != address(0), "ERC721: transfer to the zero address");
        require(
            !_isPremintedToken || !_isPremintOwner(to),
            "Premint: transfer preminted token to the Premint Owner"
        );
    ...
    function originOwnerOf(
        uint256 tokenId
    ) public view virtual returns (address) {
        return super.ownerOf(tokenId);
    }

    function premintOwner() public view virtual returns (address) {
        return _premintOwner;
    }

    function premintedCount() public view virtual returns (uint256) {
        return _premintedCount;
    }

    function isPremintedToken(
        uint256 tokenId
    ...
    function mintedCount() public view virtual returns (uint256) {
        return _mintedCount;
    }
...
contract ERC721PremintBasic is ERC721Premint, IERC721Enumerable, Ownable {
    ...
    function tokenOfOwnerByIndex(
        ...
        if (_isPremintOwner(owner_)) {
            ...
            //
            require(
                index < originBalanceOf(owner_),
                "Premint: Premint Owner index out of bounds"
            );
        }
...
abstract contract ERC721MysteryBox is ERC721PremintBasic {
    ...
    function setMaxTokens(uint256 maxTokens_) public virtual onlyOwner {
        require(
            maxTokens_ >= totalSupply() + _reservedForBox,
            "Max count is not correct"
        );
    ...
    function canMint(
        ...
        if (_maxTokens > 0) {
            require(
                _maxTokens >= totalSupply() + count + _reservedForBox,
    ...
    function mysteryBoxInfo(
        uint256 tokenId
    ) public view returns (uint256 count, string memory name, address proxy) {
    ...
    function unpack(uint256 tokenId) public {
        require(
            isMysteryBoxToken(tokenId),
            "Error: token is not a Mystery Box token"
        );
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
        ...
        if (_mapBoxesProxy[tokenId] != address(0)) {
            IProxyUnpacker proxyUnpacker = IProxyUnpacker(
                _mapBoxesProxy[tokenId]
            );
...
contract PepePunks is ERC721MysteryBox {
    ...
    ) ERC721PremintBasic("PepePunks", "PPP", premintOwner) {}
}
