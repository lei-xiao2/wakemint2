...
contract KONGS is IKONGS, ERC721Enumerable, Ownable, Pausable {
    ...
    function mint(uint256 amount) external payable whenNotPaused {
        ...
        if (whitelistMode) {
            require(
                amount <= whitelisted[_msgSender()].amount,
                "Whitelisted can mint maximum of 3"
            );
        }
    ...
    function selectRecipient(uint256 seed) internal view returns (address) {
        if (
            minted <= PAID_TOKENS ||
            (minted < 30000 && ((seed >> 245) % 10) != 0) ||
            (minted > 30000 && ((seed >> 245) % 10) >= 2)
    ...
    function getTokenTraits(
        uint256 tokenId
    ) external view override returns (KaijuKong memory) {
