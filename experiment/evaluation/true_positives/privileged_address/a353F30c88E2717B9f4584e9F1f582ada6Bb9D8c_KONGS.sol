...
interface IKongIsland {
    function addManyToKongIslandAndPack(
        address account,
...
contract KONGS is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(
        ...
        reservedMinters[0xEDa074bf977C0EF4939114d98631735819f233a4]
            .amount = 160;
    ...
    function generate(
        uint256 tokenId,
        uint256 seed
    ) internal returns (KaijuKong memory t) {
        t = selectTraits(seed);
    ...
    function selectRecipient(uint256 seed) internal view returns (address) {
        if (
            minted <= PAID_TOKENS ||
            (minted < LATE_PHASE && ((seed >> 245) % 10) != 0) ||
            (minted > LATE_PHASE && ((seed >> 245) % 10) >= 2)
    ...
    function selectTraits(
        uint256 seed
    ) internal view returns (KaijuKong memory t) {
    ...
    function getTokenTraits(
        uint256 tokenId
    ) external view returns (KaijuKong memory) {
    ...
    function KingScore(uint256 tokenId) external view returns (uint8) {
        require(ecosystem[msg.sender] || mintBlock[tokenId] + 1 < block.number);
        if (tokenTraits[tokenId].isKaiju) {
            return 0;
        }
        if (tokenTraits[tokenId].kingScore == 5) {
            return 0;
        } else if (tokenTraits[tokenId].kingScore == 4) {
            return 1;
        } else if (tokenTraits[tokenId].kingScore == 3) {
            return 2;
        } else {
            return 3;
        }
    ...
    function balanceOf(
        ...
        require(
            ecosystem[msg.sender] || addressLastMint[owner] + 1 < block.number
        );
    ...
    function tokenOfOwnerByIndex(
        address owner,
        uint256 index
    ) public view virtual override(ERC721Enumerable) returns (uint256) {
        require(
            ecosystem[msg.sender] || addressLastMint[owner] + 1 < block.number
        );
    ...
    function getIDsOwnedby(
        address _address
    ) external view returns (uint256[] memory) {
        require(
            ecosystem[msg.sender] ||
                addressLastMint[_address] + 1 < block.number
        );
    ...
    function bulkAddReserved(
        address[] calldata _reserved,
