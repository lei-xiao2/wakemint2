...
contract JSNowLoading is StandardERC721, Mintable {
    ...
    function bulkMint(
        ...
        require(
            maxTokenId < _fromTokenId,
            "ERC721: tokenId range is already minted"
        );
    ...
    function ownerOf(uint256 tokenId) public view override returns (address) {
        address tokenOwner = _owners[tokenId];
        require(
            tokenOwner != address(0) || tokenId <= totalSupply,
            "ERC721: owner query for nonexistent token"
        );
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        string
            memory baseURL = "https://raw.githubusercontent.com/js-nowloading/asset/main/metadata/";
        return
            string(
                abi.encodePacked(baseURL, Strings.toString(_tokenId), ".json")
            );
    }
