...
library Address {
    ...
    function toString(address account) internal pure returns (string memory) {
        ...
        for (uint i = 0; i < 20; i++) {
            bytes1 b = bytes1(
                uint8(uint(uint160(account)) / (2 ** (8 * (19 - i))))
            );
    ...
    IBanning
{
    ...
    bytes32 public whitelistedRoot =
        0xa6e4dad26037d78e4f5abac1b4b3cd9f941d30623c13526d8e8b1b2ba1341b57;
    bytes32 public giveawayRoot =
    ...
    function _setLocked(
        ...
        require(ownerOf(tokenId) == _user, "lock of token that is not own");
    ...
    function giveawayMint(
        ...
        require(
            giveawaysOf(_to, _total, _merkleProof) >= _mintAmount,
            "insufficient giveaways"
        );
    ...
    function isApprovedForAll(
        ...
        if (msg.sender != owner() && _owner != owner()) {
            ...
            if (
                isBanned(msg.sender) || isBanned(_owner) || isBanned(_operator)
            ) {
                return false;
            }
        }
    ...
    function giveawaysOf(
        ...
        require(_to != address(0), "giveaways query for the zero address");

        bytes32 leaf = keccak256(abi.encodePacked(_to, _total));
        require(
            MerkleProof.verify(_merkleProof, giveawayRoot, leaf),
            "invalid proof"
        );
    ...
    function gamesOf(
        ...
        require(_to != address(0), "games query for the zero address");
        return _games[_to];
    }
    ...
    function setProxyRegistryAddress(
        address _newProxyRegistryAddress
    ) public onlyOwner {
    ...
    function setWhitelistedCost(
        uint256 _newWhitelistedCost
    ...
    function setWhitelistedRoot(
        bytes32 _newWhitelistedRoot
    ) public virtual override onlyOwner {
        whitelistedRoot = _newWhitelistedRoot;
    }

    function setGiveawayRoot(
        bytes32 _newGiveawayRoot
    ) public virtual override onlyOwner {
    ...
    function locked(
        ...
        _setLocked(
            _user,
            tokenId,
            _state ? code : _lockedCode[_user][tokenId],
            _state
        );
    }
    ...
    function addGames(
        ...
        require(_to != address(0), "add games for the zero address");
    ...
    function removeGames(
        ...
        require(_to != address(0), "remove games for the zero address");
        _games[_to] -= _newGames;
    }
