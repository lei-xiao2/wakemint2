...
contract Contract is Master, IAsset {
    ...
    function safeMint(address _owner, uint256 _quantity) external {
        RootOnly();
        CheckMintStatus();
        ActiveMint();

        if (_root != _owner) _checkMint(_owner, _quantity);
    ...
    function _checkMint(address _owner, uint256 _quantity) private view {
        if (
            !quantityIsGood(
                _quantity,
                _numberMinted(_owner),
                _numberMintedOnPresale(_owner)
            )
        ) revert OutOfMintBoundaries();
        if (!supplyIsGood()) revert OutOfMintBoundaries();
    }

    function locked(uint256 _assetId) external view override returns (bool) {
        return
    ...
    function unlock(uint256 _assetId, address _owner) external {
        address sender = _msgSender();
        if (
            _root == sender ||
