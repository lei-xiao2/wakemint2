                ...
                if (ownership.addr == _owner) {
                    if (!ownership.burned) {
                        if (_contractData.isEnvelope) {
                            ...
                            if (
                                !IEnvelope(_envelopeTypes.envelope).locked(
                                    address(this),
                                    i
                                )
                            ) {
...
contract AccessControl is Ownership {
    function ActiveMint() internal view {
        if (MintStatus.NONE == _contractData.mintStatus)
            revert MintShouldBeOpened();
    }

    function ApprovedOnly(address owner) internal view {
        if (!_operatorApprovals[owner][_msgSender()])
            revert CallerNotOwnerNorApproved();
    }

    function BotProtection() internal view {
        if (tx.origin != msg.sender) revert Err();
    }

    function OwnerOnly(address owner, uint256 tokenId) internal view {
        if (owner != ownershipOf(tokenId).addr)
            revert CallerNotOwnerNorApproved();
    }

    function RootOnly() internal view {
        address sender = _msgSender();
        if (sender != _root && sender != _envelopeTypes.envelope)
            revert RootAddressError();
    }

    function Whitelisted(bytes32[] calldata _merkleProof) internal view {
        address sender = _msgSender();
        bool flag = _root == sender ||
        ...
        /**/
        if (!flag) revert WhitelistedOnly();
    }

    function setWLRoot(bytes32 _root) external {
        RootOnly();
...
contract Array {
    function remove(uint256[] memory arr, uint256 e) internal pure {
        unchecked {
            uint idx = 0;
...
contract Payment is TokenStorage {
    function lackOfMoney(uint _quantity) internal returns (bool) {
        return
            msg.value <
            Math.mul(
                _contractData.mintStatus == MintStatus.PRESALE
                    ? _mintSettings.priceOnPresale
                    : _mintSettings.priceOnSale,
                _quantity
            );
    }

    function lackOfMoneyForConcat() internal returns (bool) {
        return
            _mintSettings.envelopeConcatPrice != 0 &&
            _mintSettings.envelopeConcatPrice > msg.value;
    }

    function lackOfMoneyForSplit() internal returns (bool) {
        return
...
contract Quantity is TokenStorage {
    function quantityIsGood(
        uint256 _quantity,
        uint256 _minted,
        uint256 _mintedOnPresale
    ) internal view returns (bool) {
        return
            (_contractData.mintStatus == MintStatus.PRESALE &&
                _mintSettings.mintOnPresale >= _quantity + _minted) ||
            (_contractData.mintStatus == MintStatus.SALE &&
                _mintSettings.maxMintPerUser >=
                _quantity + _minted - _mintedOnPresale &&
                _mintSettings.minMintPerUser <= _quantity);
    }

    function supplyIsGood() internal view returns (bool) {
        return
    ...
    Quantity
{
    ...
    function _numberMinted(address owner) internal view returns (uint256) {
        ...
        else return uint256(_addressData[owner].numberMinted);
    }

    function _numberMintedOnPresale(
        address owner
    ) internal view returns (uint256) {
        if (owner == address(0)) revert MintedQueryForZeroAddress();
        else return uint256(_addressData[owner].numberMintedOnPresale);
    }
    ...
    function _numberBurned(address owner) internal view returns (uint256) {
        ...
        else return uint256(_addressData[owner].numberBurned);
    }
    ...
    function getApproved(
        ...
        else return _tokenApprovals[tokenId];
    }
    ...
    function _mint(
        ...
        if (!supplyIsGood()) revert OutOfMintBoundaries();
        ...
        unchecked {
            ...
            if (_contractData.mintStatus == MintStatus.PRESALE)
                _addressData[to].numberMintedOnPresale =
                    _addressData[to].numberMintedOnPresale +
                    uint64(quantity);
            ...
            for (uint i = 0; i < _envelopeTypes.types.length; i++) {
                (bool success, bytes memory res) = _envelopeTypes.types[i].call(
                    abi.encodeWithSignature(
                        "unlock(uint256,address)",
                        _assetsEnvelope[_assetId][_envelopeTypes.types[i]],
                        _to
                    )
                );
                if (!success) revert AssetCannotBeTransfered();
            }
        }
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal {
        ...
        bool isApprovedOrOwner = (sender == _envelopeTypes.envelope ||
    ...
    function getBalance() external view returns (uint256) {
        if (_root != _msgSender()) revert RootAddressError();
        return address(this).balance;
    }

    function withdraw(address _to, uint256 _amount) external {
        if (_root != _msgSender()) revert RootAddressError();
        if (address(this).balance < _amount) revert LackOfMoney();
...
abstract contract ERC721AEnvelope is Array, Context, ERC721AToken {
    ...
    function _mintSetOfAssets(address _owner, uint _quantity) internal {
        unchecked {
            for (uint i = 0; i < _envelopeTypes.types.length; i++) {
                (bool success, bytes memory res) = _envelopeTypes.types[i].call(
                    abi.encodeWithSignature(
    ...
    function _envelopeAssets(
        ...
        unchecked {
            ...
            for (uint i = 0; i < _envelopeTypes.types.length; i++) {
                ...
                tokens[len++] = _assetsEnvelope[_envelopeId][
                    _envelopeTypes.types[i]
                ];
            }
            return (addrs, tokens);
        }
    ...
    function _envelopeSplit(
        address _owner,
        uint256 _envelopeId
    ) internal returns (address[] memory, uint256[] memory) {
        OwnerOnly(_owner, _envelopeId);

        (address[] memory addrs, uint256[] memory tokens) = _envelopeAssets(
            _envelopeId
        );
        _burn(_envelopeId);
        ...
        unchecked {
            for (uint i = 0; i < addrs.length; i++) {
                _unlockEnvelopeAsset(_envelopeId, addrs[i], tokens[i]);
            }
        }
        return (addrs, tokens);
    }

    function _unlockEnvelopeAsset(
        uint256 _envelopeId,
        address _asset,
        uint256 _assetId
    ) internal {
        if (!_locked(_asset, _assetId)) revert AssetNotLocked();
    ...
    function _envelopeCreate(
        address _owner,
        address[] calldata _assets,
        uint256[] calldata _assetIds
    ) internal returns (uint256) {
        if (_assets.length == 0 && _assets.length != _assetIds.length)
            revert Err();
        ...
        unchecked {
            ...
            for (uint i = 0; i < _assets.length; i++) {
                if (_locked(_assets[i], _assetIds[i])) revert AssetLocked();
    ...
    function _locked(
        ...
        if (_contractData.isEnvelope) return _assetsEnveloped[_asset][_assetId];
        else return IAsset(_envelopeTypes.envelope).locked(_assetId);
    }
}
...
abstract contract Master is ERC721AEnvelope {
    ...
    function setRoot(address _owner) external {
        RootOnly();
    ...
    function CheckMintStatus() internal {
        if (!_contractData.mintStatusAuto) return;
    ...
    function toggleMintStatus(bool _mode) external {
        RootOnly();

        _contractData.mintStatusAuto = _mode;
    }

    function setMintingIsOnPresale() external {
        RootOnly();

        _contractData.mintStatus = MintStatus.PRESALE;
    }

    function setMintingIsOnSale() external {
        RootOnly();
    ...
    function setRevealed(string calldata _url) external {
        RootOnly();
    ...
    function updateBaseURL(string calldata _url) external {
        RootOnly();
...
contract Contract is Master, Payment, IEnvelope {
    constructor(
        ...
        string memory baseURL_,
        string memory contractURL_
    ) ERC721A(name_, description_, symbol_, baseURL_, contractURL_) Master() {
        _contractData.isEnvelope = true;
        //_contractData.wl = 0x7355b511eb06aa6d5a11b366b27ed407bc3237cf6e2eafe1799efef4a678756f;
        _contractData
            .wl = 0xcdab47e163c1eb6040f36523ce1ddb86b732c6e652e159613a6e0b896d4f8232;
    }

    function addAssetType(address _asset) external {
        RootOnly();
    ...
    function setEnvelopeConcatPrice(uint256 _price) external {
        RootOnly();

        _mintSettings.envelopeConcatPrice = _price;
    }

    function setEnvelopeSplitPrice(uint256 _price) external {
        RootOnly();

        _mintSettings.envelopeSplitPrice = _price;
    }

    function addMint(uint _quantity) external payable returns (uint256) {
        BotProtection();
        CheckMintStatus();
        ActiveMint();
    ...
    function addMint(address _owner, uint _quantity) external {
        RootOnly();
        CheckMintStatus();
    ...
    function envelopeCreate(
        ...
        if (lackOfMoneyForConcat()) revert LackOfMoney();
        else return _envelopeCreate(_msgSender(), _assets, _assetIds);
    }

    function envelopeSplit(
        uint256 _envelopeId
    ) external payable returns (address[] memory, uint256[] memory) {
        OwnerOnly(_msgSender(), _envelopeId);

        if (lackOfMoneyForSplit()) revert LackOfMoney();
        else return _envelopeSplit(_msgSender(), _envelopeId);
    }

    function getAssetTypes() external view returns (address[] memory) {
        return _envelopeTypes.types;
    }

    function getEnvelopeAssets(
        uint256 _envelopeId
