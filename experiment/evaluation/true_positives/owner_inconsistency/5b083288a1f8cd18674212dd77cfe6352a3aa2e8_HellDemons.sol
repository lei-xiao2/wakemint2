...
contract HellDemons is ERC721, Ownable {
    ...
    function createTokenURI(
        ...
        return
            CustomString.strConcat(
                baseTokenURI,
                CustomString.uint2str(tokenId)
            );
    }
    ...
    function mintDemon(uint256 _numberOfTokens) public payable {
        ...
        if (presale) {
            ...
            require(
                _numberOfTokens <= presaleMaxMintNumber,
                "Unable to mint exceed number of demons per mint."
            );
            require(
                _tokenBalance[_msgSender()].add(_numberOfTokens) <=
                    presaleMaxMintNumber,
                "You have a demon already."
            );
            require(
                currentSupply.add(_numberOfTokens) <
                    presaleAmount.add(numOfGiveaway).add(6),
                "No tokens available for presale"
            );
            price = presalePrice;
        } else {
            require(!paused, "Minting is paused");
            require(
                _numberOfTokens <= maxMintNumber,
                "Too many tokens to mint at once."
            );
            require(
                _tokenBalance[_msgSender()].add(_numberOfTokens) <=
                    maxMintNumber,
                "You have enough demons already."
            );
            require(
                currentSupply.add(_numberOfTokens) <= totalSupply,
    ...
    function withdraw() public onlyOwner {
        (bool hs, ) = payable(devWallet).call{
            value: (address(this).balance * devFee) / 1000
        }("");
        require(hs);
