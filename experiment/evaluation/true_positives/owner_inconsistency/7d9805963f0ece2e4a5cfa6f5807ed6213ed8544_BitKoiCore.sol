...
abstract contract BitKoiTraitInterface {
    ...
    function isBitKoiTraits() public pure virtual returns (bool);
...
abstract contract BitKoiBreeding is BitKoiOwnership {
    ...
    function _triggerCooldown(BitKoi storage _koiFish) internal {
        ...
        _koiFish.cooldownEndBlock = uint64(
            (cooldowns[_koiFish.cooldownIndex] / secondsPerBlock) + block.number
        );
    ...
    function isReadyToHatch(uint256 _koiId) public view returns (bool) {
        require(_koiId > 0);
    ...
    ///  in the middle of a cooldown).
    /// @param _koiId reference the id of the fish, any user can inquire about it
    function isReadyToBreed(uint256 _koiId) public view returns (bool) {
        require(_koiId > 0);
    ...
    function _isValidMatingPair(
        ...
        if (
            _parent1.parent1Id == _parent2Id || _parent1.parent2Id == _parent2Id
        ) {
            return false;
        }
        if (
            _parent2.parent1Id == _parent1Id || _parent2.parent2Id == _parent1Id
        ) {
            return false;
        }
        ...
        if (
            _parent2.parent1Id == _parent1.parent1Id ||
            _parent2.parent1Id == _parent1.parent2Id
        ) {
            return false;
        }

        if (
            _parent2.parent2Id == _parent1.parent1Id ||
            _parent2.parent2Id == _parent1.parent2Id
        ) {
            return false;
        }
    ...
    function _breedWith(
        ...
        uint256 newFishId = _createBitKoi(
            _parent1Id,
            _parent2Id,
            parentGen + 1,
            0,
        ...
        emit BreedingSuccessful(
            bitKoiIndexToOwner[_parent1Id],
            newFishId,
            _parent1Id,
            _parent2Id,
            parent1.cooldownEndBlock
        );
    ...
    function breedWith(
        ...
        require(_isValidMatingPair(parent1, _parent1Id, parent2, _parent2Id));
    ...
    function _hatchFish(
        ...
        uint256 newFishGenes = bitKoiTraits.smooshFish(
            genes1,
            genes2,
            fishToHatch.cooldownEndBlock - 1
        );
...
abstract contract BitKoiSale is BitKoiBreeding {
    ...
    function getSale(
        uint256 _tokenId
    ) external view returns (address seller, uint256 price, uint256 startedAt) {
        Sale storage sale = tokenIdToSale[_tokenId];
        require(_isOnSale(sale));
        return (sale.seller, sale.price, sale.startedAt);
    }
...
contract BitKoiCore is BitKoiMinting {
    constructor(
        address _proxyRegistryAddress
    ) ERC721Tradable("BitKoi", "BITKOI", _proxyRegistryAddress) {
        // Starts paused.
        paused = true;
