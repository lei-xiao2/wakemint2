...
contract ChubbyBunny is ERC721, Ownable {
    ...
    function mintPunk(uint256 _numberOfTokens) public payable {
        ...
        if (seasonState == 1) {
            require(
                currentSupply.add(_numberOfTokens) <= season1,
                "No punks available for minting!"
            );
        } else {
            require(
                currentSupply.add(_numberOfTokens) <= season1.add(season2),
                "No punks available for minting!"
            );
        }
        ...
        if (presale) {
            require(
                currentSupply.add(_numberOfTokens) <= maxPresale,
                "Presale is disabled!"
            );
            if (currentSupply.add(_numberOfTokens) >= maxPresale)
                presale = false;
            price = presalePrice;
        } else {
