...
contract Azimuth is Ownable {
    ...
    function getSpawned(
        uint32 _point
    ) external view returns (uint32[] spawned) {
    ...
    function getSponsoring(
        uint32 _sponsor
    ) external view returns (uint32[] sponsees) {
    ...
    function isRequestingEscapeTo(
        uint32 _point,
        uint32 _sponsor
    ) public view returns (bool equals) {
        Point storage point = points[_point];
    ...
    function getEscapeRequests(
        uint32 _sponsor
    ) external view returns (uint32[] requests) {
    ...
    function getOwnedPoints(
        address _whose
    ) external view returns (uint32[] ownedPoints) {
    ...
    function getManagerFor(
        address _proxy
    ) external view returns (uint32[] mfor) {
    ...
    function getSpawningFor(
        address _proxy
    ) external view returns (uint32[] sfor) {
    ...
    function getVotingFor(
        address _proxy
    ) external view returns (uint32[] vfor) {
    ...
    function getTransferringFor(
        address _proxy
    ) external view returns (uint32[] tfor) {
...
contract Ecliptic is EclipticBase, SupportsInterfaceWithLookup, ERC721Metadata {
    ...
    function adopt(uint32 _point) external {
        require(
            azimuth.isEscaping(_point) &&
                azimuth.canManage(azimuth.getEscapeRequest(_point), msg.sender)
        );
    ...
    function setSpawnProxy(
        uint16 _prefix,
        address _spawnProxy
    ) external activePointSpawner(_prefix) {
    ...
    //
    function setTransferProxy(uint32 _point, address _transferProxy) public {
