...
contract EtheremonDataBase {
    ...
    function withdrawEther(
        address _sendTo,
        uint _amount
    ) external returns (EtheremonEnum.ResultCode);
    ...
    function removeMonsterIdMapping(
        address _trainer,
        uint64 _monsterId
    ) external;
    function addMonsterIdMapping(address _trainer, uint64 _monsterId) external;
    function clearMonsterReturnBalance(
        uint64 _monsterId
    ) external returns (uint256 amount);
    function collectAllReturnBalance(
        address _trainer
    ) external returns (uint256 amount);
    ...
    function getMonsterName(
        uint64 _objId
    ) external constant returns (string name);
    function getExtraBalance(
        address _trainer
    ) external constant returns (uint256);
    function getMonsterDexSize(
        address _trainer
    ) external constant returns (uint);
    function getMonsterObjId(
        address _trainer,
        uint index
    ...
    function getMonsterReturn(
        uint64 _objId
    ) external constant returns (uint256 current, uint256 total);
}

interface EtheremonBattle {
    function isOnBattle(uint64 _objId) external constant returns (bool);
}

interface EtheremonTradeInterface {
    function isOnTrading(uint64 _objId) external constant returns (bool);
}
    ...
    BasicAccessControl
{
    ...
    // internal function
    function _canOperate(address _tokenOwner) internal constant {
        require(
            _tokenOwner == msg.sender ||
                ownerToOperators[_tokenOwner][msg.sender]
        );
    }
    ...
    function _canTransfer(
        ...
        require(
            !battle.isOnBattle(uint64(_tokenId)) &&
                !trade.isOnTrading(uint64(_tokenId))
        );
    ...
    function isApprovable(
        ...
        EtheremonBattle battle = EtheremonBattle(battleContract);
        EtheremonTradeInterface trade = EtheremonTradeInterface(tradeContract);
        return (!battle.isOnBattle(obj.monsterId) &&
            !trade.isOnTrading(obj.monsterId));
    }
    ...
    function approve(address _approved, uint256 _tokenId) external {
        ...
        EtheremonTradeInterface trade = EtheremonTradeInterface(tradeContract);
        if (
            battle.isOnBattle(obj.monsterId) || trade.isOnTrading(obj.monsterId)
        ) revert();
...
contract EtheremonMonsterEnumerable is
...
contract EtheremonMonsterStandard is
    ...
    EtheremonMonsterStandard("EtheremonMonster", "EMONA")
{
    ...
    function mintMonster(
        ...
        address _trainer,
        string _name
    ) external onlyModerators returns (uint) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        // add monster
        ...
        for (uint i = 0; i < STAT_COUNT; i += 1) {
            value =
                ...
                data.getElementInArrayType(
                    EtheremonEnum.ArrayType.STAT_START,
                    uint64(_classId),
                    i
                );
            data.addElementToArrayType(
                EtheremonEnum.ArrayType.STAT_BASE,
                objId,
                value
            );
        }
    ...
    function getMonsterBaseStats(
        ...
        external
        constant
        returns (uint hp, uint pa, uint pd, uint sa, uint sd, uint speed)
    {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        uint[6] memory stats;
        for (uint i = 0; i < STAT_COUNT; i += 1) {
        ...
        )
    {
        ...
        for (i = 0; i < STAT_COUNT; i++) {
            stats[i] +=
                uint(
                    data.getElementInArrayType(
                        EtheremonEnum.ArrayType.STAT_STEP,
                        obj.classId,
                        i
                    )
                ) *
    ...
    function getMonsterCP(
        uint64 _monsterId
    ) external constant returns (uint cp) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterObjAcc memory obj;
        (
            obj.monsterId,
    ...
    function getPrice(
        uint32 _classId
    ) external constant returns (bool catchable, uint price) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
    ...
    function getMonsterClassBasic(
        uint32 _classId
    ) external constant returns (uint256, uint256, uint256, bool) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterClassAcc memory class;
        (
            class.classId,
    ...
    function catchMonster(
        ...
        uint32 _classId,
        string _name
    ) external payable isActive returns (uint tokenId) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterClassAcc memory class;
        (
            class.classId,
        ...
        if (class.catchable == false) {
            if (
                addressWhitelist[msg.sender] == false ||
