...
/**
 ...
 ____  __ __ _____ _____ _____ _____    ___  __  _____  _____ __ _____   __ 
(( ___ || ||  ||    ||   ||==  ||_//    || \/ | ((   )) ||_// || ||==   ((  
 \\_|| \\_//  ||    ||   ||___ || \\    ||    |  \\_//  || \\ || ||___ \_)) 
...
/**
 * @title Gutter Mories contract.
 * @author The Gutter Punks team.
 *
 */
contract GutterMories is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public constant GP_OFFSET = 9000;
    uint256 public constant MAX_SUPPLY = 1111;
    uint256 public REWARD_AIRDROPS = 0;
    uint256 public ADDITIONAL_PUNKS = 0;
    uint256 public ADDITIONAL_PUNKS_START = 1000;
    ...
    address internal _allowedAirdropper =
        0xcc2e2248216985Cd8b96E3bf9aABF388B09cED05;
    ...
    function setGutterPunksTargetContract(
    ...
    function setAllowedAirdropper(
        address allowedAirdropper
    ) external onlyOwner {
    ...
    function airdrop(address[] calldata recipients) external onlyOwner {
        ...
        require(
            startingSupply + recipients.length <= MAX_SUPPLY,
            "Cannot airdrop more than max supply."
        );
    ...
    function airdropAdditional(address[] calldata recipients) external {
        ...
        require(
            msg.sender == _allowedAirdropper || msg.sender == owner(),
            "Not authorized."
        );
        require(
            startingTokenId + recipients.length - 1 <= MAX_SUPPLY,
            "Out of additional airdrop tokens."
        );
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        if (tokenId > MAX_SUPPLY) {
            return false;
        } else if (_owners[tokenId] == _burnAddress) {
            return false;
        } else if (_owners[tokenId] != address(0)) {
            return true;
        } else if (tokenId > 0 && tokenId <= 999) {
    ...
    function _ownerOf(uint256 tokenId) internal view virtual returns (address) {
        ...
        if (owner == address(0) && tokenId > 0 && tokenId <= 999) {
            try _gpTarget.ownerOf((tokenId + GP_OFFSET)) returns (
                address result
            ) {
                owner = result;
            } catch {
    ...
    function parentTokenTransferred(
        address from,
        uint256 tokenId
    ) public virtual {
        require(
            _msgSender() == _gpAddress,
            "This function must be called by the airdrop token parent."
        );
        // Only update token owner if current owner is unset
        if (tokenId >= 9001 && tokenId <= 9999) {
            tokenId = tokenId - GP_OFFSET; // offset for GP reward batch GP #9001 -> GM #1
