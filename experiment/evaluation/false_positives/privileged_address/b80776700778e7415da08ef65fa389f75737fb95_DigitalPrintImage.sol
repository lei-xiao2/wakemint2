...
contract IAssetManager {
    ...
    function checkHasPermissionForPack(
        address _address,
        uint _packId
    ) public view returns (bool);
    ...
    function getAssetPacksUserCreated(
        address _address
    ...
    function getAssetPackData(
        uint _assetPackId
    ) public view returns (string, uint[], uint[], bytes32[]);
...
contract Functions {
    ...
    function calculateSeed(
        ...
        bytes32 randomSeed = keccak256(
            abi.encodePacked(
                randomHashes[_randomHashIds[0]],
                randomHashes[_randomHashIds[1]],
                randomHashes[_randomHashIds[2]],
                randomHashes[_randomHashIds[3]],
                randomHashes[_randomHashIds[4]],
                randomHashes[_randomHashIds[5]],
                randomHashes[_randomHashIds[6]],
                randomHashes[_randomHashIds[7]],
                randomHashes[_randomHashIds[8]],
                randomHashes[_randomHashIds[9]],
                _timestamp
            )
        );
    ...
    ) public pure returns (uint[] assets) {
    ...
    function pickRandomAssets(
        uint _finalSeed,
        bytes32[] _potentialAssets
    ) public pure returns (uint[] finalPicked) {
        ...
        for (uint i = 0; i < assetIds.length; i++) {
            finalSeedCopy = uint(
                keccak256(abi.encodePacked(finalSeedCopy, assetIds[i]))
            );
        ...
        for (uint i = 0; i < assetIds.length; i++) {
            ...
            if (finalSeedCopy % 2 == 0) {
                ...
                (
                    ...
                    rotation[index],
                    layers[index]
                ) = pickRandomAssetPosition(finalSeedCopy, _width, _height);
                index++;
            }
        }
    ...
    function pickRandomAssetPosition(
        uint _randomSeed,
        uint _width,
        uint _height
    )
...
contract UserManager {
    ...
    function register(string _username, bytes32 _hashToProfilePicture) public {
        require(
            usernameExists[_username] == false ||
                keccak256(abi.encodePacked(getUsername(msg.sender))) ==
    ...
    function getUserInfo(
        address _address
    ) public view returns (string, bytes32) {
...
contract DigitalPrintImage is
    ERC721Token("DigitalPrintImage", "DPM"),
    UserManager,
    Ownable
{
    ...
    function createImage(
        ...
        require(
            msg.sender == usernameToAddress[_author] || !usernameExists[_author]
        );
        ...
        (pickedAssets, finalSeed) = getPickedAssetsAndFinalSeed(
            _potentialAssets,
            _randomHashIds,
            _timestamp,
            _iterations
        );
        uint[] memory pickedAssetPacks = assetManager.pickUniquePacks(
            pickedAssets
        );
        uint finalPrice = 0;
        ...
        for (uint i = 0; i < pickedAssetPacks.length; i++) {
            if (
                assetManager.checkHasPermissionForPack(
                    msg.sender,
                    pickedAssetPacks[i]
                ) == false
            ) {
                finalPrice += assetManager.getAssetPackPrice(
                    pickedAssetPacks[i]
                );

                assetManager.buyAssetPack.value(
                    assetManager.getAssetPackPrice(pickedAssetPacks[i])
                )(msg.sender, pickedAssetPacks[i]);
            }
        }
    ...
    function transferFromMarketplace(
        ...
        address _to,
        uint256 _imageId
    ) public onlyMarketplaceContract {
    ...
    function addMarketplaceContract(
    ...
    function getGalleryData(
        uint _imageId
    ) public view returns (address, address, string, bytes32, string, string) {
    ...
    function getImageMetadata(
        uint _imageId
    ) public view returns (address, string, uint, string, uint, bytes32[]) {
    ...
    function getPickedAssetsAndFinalSeed(
        bytes32[] _potentialAssets,
        uint[] _randomHashIds,
        uint _timestamp,
        uint _iterations
        ...
        uint finalSeed = uint(
            functions.getFinalSeed(
                functions.calculateSeed(_randomHashIds, _timestamp),
                _iterations
            )
        );
        ...
        return (
            functions.pickRandomAssets(finalSeed, _potentialAssets),
            finalSeed
        );
    }
}
