...
interface VVVStaking {
    ...
    function getStakingItemInfo(
        address _user,
        uint256 _tokenId
    ) external view returns (bool _isStaked, uint _startTime, uint _lockTime);
}
...
contract VVVS1R is ERC721Enumerable, Ownable {
    ...
    function getLockTime(uint256 tokenId) public view returns (string memory) {
        (, , uint256 _lockTime) = stakingContract.getStakingItemInfo(
            ownerOf(tokenId),
            tokenId
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(
            to == address(stakingContract) || from == address(stakingContract),
            "This token can't transfer to other address"
        );
        require(
            msg.sender == address(stakingContract) || msg.sender == owner(),
            "Only transferable by staking or unstaking VVVS1"
        );
        super._beforeTokenTransfer(from, to, tokenId);
    }
}
