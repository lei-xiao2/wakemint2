...
contract BattleMancers is
    ERC721A,
    BattleMancersAdministration,
    ReentrancyGuard
{
    ...
    constructor(
        address _operator
    ) BattleMancersAdministration(_operator) ERC721A("BattleMancers", "BM") {
    ...
    modifier publicSaleActive() {
        if (publicMintTime == 0 || block.timestamp < publicMintTime)
            revert SaleNotActive("Public Sale");
        _;
    }
    modifier whitelistSaleActive() {
        if (whitelistMintTime == 0 || block.timestamp < whitelistMintTime)
            revert SaleNotActive("Whitelist Sale");
        _;
    }
    modifier notSoldOut() {
        ...
        if ((_totalMinted() - _reserveMinted) >= _avaliableForUsers)
            _soldOut = true;
    }
    // =============================================================
    //                   MINT FUNCTIONS
    ...
    function mint(
        uint256 quantity
    ) external payable publicSaleActive notSoldOut nonReentrant {
        // Calculate avaliable mints
        uint256 userMintsRemaining = getRemainingQuantity(msg.sender);
        require(
            userMintsRemaining > 0,
            "No more public mints avaliable for this address"
        );

        // Actual mint amount
        uint256 toMint = (quantity > userMintsRemaining)
            ? userMintsRemaining
            : quantity;
    ...
    ) external payable whitelistSaleActive notSoldOut nonReentrant {
    ...
    function getRemainingQuantity(address user) public view returns (uint256) {
        ...
        uint256 userMintsRemaining = _avaliableForUsers -
            (_totalMinted() - _reserveMinted);
    ...
    function mintReserved(
        ...
        require(
            (_reserveMinted + quantity) <= RESERVE_SUPPLY,
            "Not enough reserve mints avaliable"
        );
